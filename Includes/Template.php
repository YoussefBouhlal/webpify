...
final class Template {
	...
	public static function get_part( $slug, $name = '' ) {
		...
		$template = apply_filters( 'webpify_get_template_part', $template, $slug, $name );
	...
	public static function get( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
		...
		$located = apply_filters( 'webpify_get_template', $located, $template_name, $args, $template_path, $default_path );

		// Perform other actions before template part is included.
		do_action( 'webpify_before_template_part', $template_name, $template_path, $located, $args );

		include $located;

		// Perform other actions after template part is included.
		do_action( 'webpify_after_template_part', $template_name, $template_path, $located, $args );
	}
	...
	public static function locate( $template_name, $template_path = '', $default_path = '' ) {
		...
		return apply_filters( 'webpify_locate_template', $template, $template_name, $template_path );
	}
}
